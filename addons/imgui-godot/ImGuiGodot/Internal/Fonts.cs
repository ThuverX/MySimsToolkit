...
internal sealed class Fonts
{
    ...
    public void AddFont(FontFile? fontData, int fontSize, bool merge, ushort[]? ranges, bool isIcon)
    {
    ...
    private static unsafe void AddFontToAtlas(FontParams fp, float scale)
    {
        ...
        if (fp.IsIcon)
        {
            if (fp.Merge) fc->MergeMode = 2;
            fc->GlyphOffset = new Vector2(0, 1);
        }
        ...
        else
        {
            ...
            {
                fixed (ushort* source = &new ushort[3] { fp.Ranges[0], fp.Ranges[1], 0 }[0])
                {
                    var iconFontRanges = Marshal.AllocHGlobal(6);
                    Buffer.MemoryCopy(source, iconFontRanges.ToPointer(), 6L, 6L);
                    fc->GlyphRanges = (ushort*)iconFontRanges.ToPointer();
                    io.Fonts.AddFontFromMemoryTTF(p, len, fontSize, fc, iconFontRanges);
                }
            }
        }
