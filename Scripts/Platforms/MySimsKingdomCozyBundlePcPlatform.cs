...
public class MySimsKingdomCozyBundlePcPlatform : GamePlatform
{
    public override GameType GameType => GameType.MySimsKingdomCozyBundlePc;
    public override bool HasSaveLocation => true;
    public override string SaveLocation => "%LOCALAPPDATA%/Electronic Arts/MySimsKingdom";

    public Dictionary<string, uint> ResourceTypes = [];

    public override bool IsValidPath(string path)
    {
        return File.Exists(Path.Join(path, "MySimsKingdom.exe")) && Directory.Exists(Path.Join(path, "data"));
    }
    ...
    {
        ResourceTypes = [];
        var mapPath = Path.Join(rootPath, "data", "DDFMap.txt");
    ...
    private Task<IMaterial> MaterialLoader(IFileSystem.FileId fileId)
    {
        return JobService.Instance.Run(IMaterial () =>
        {
            ...
            return MaterialData.Read(reader, MaterialVersion.Kingdom);
        });
    }
    ...
    private Task<IMaterialSet> MaterialSetLoader(IFileSystem.FileId fileId)
    {
        return JobService.Instance.Run(IMaterialSet () =>
        {
            ...
            if (type == ResourceTypes["materialset"])
            {
                ...
                return MaterialSet.Read(reader, MaterialVersion.Kingdom);
            }
